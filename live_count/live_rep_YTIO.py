...
class RepDetector:
    ...
    def get_boundingbox(self):
        ...
        if framesstd == 0:  # no bb, take full frame
            ...
            for i in range(20):
                frameROIRes[i, :, :] = scipy.misc.imresize(
                    self.frame_set[i, :, :], size=(50, 50), interp="bilinear"
                )

            frameROIRes = numpy.reshape(
                frameROIRes,
        ...
        for i in range(20):
            frameROIRes[i, :, :] = scipy.misc.imresize(
                framesRoi[i, :, :], size=(50, 50), interp="bilinear"
            )
    ...
    def do_local_count(self, classify, initial):
        ...
        self.label_array = numpy.insert(
            self.label_array, history_num - 1, output_label, axis=0
        )
        # take median of the last frames
        med_out_label = numpy.ceil(
            numpy.median(self.label_array[history_num - 4 : history_num])
        )
    ...
    def count(self, proFrame):
        ...
        self.count_array = numpy.insert(
            self.count_array, history_num - 1, self.rep_count, axis=0
        )
        self.ent_arr = numpy.delete(self.ent_arr, 0, axis=0)
        self.ent_arr = numpy.insert(
            self.ent_arr, history_num - 1, self.cur_entropy, axis=0
        )
...
def draw_str(dst, xxx_todo_changeme, s, color, scale):
    ...
    cv2.putText(
        dst, s, (x, y), cv2.FONT_HERSHEY_PLAIN, scale, (255, 255, 255), lineType=11
    )
...
if __name__ == "__main__":
    ...
    layer4 = LogisticRegression(
        input=layer3.output, n_in=500, n_out=8
    )  # change the number of output labels
        ...
        while True:
            ...
            if (cur_state == state.IN_REP) and (
                ...
                draw_str(
                    frame,
                    (20, 120),
                    " new hypothesis (%d) " % global_counter,
                    orange_color,
                    1.5,
                )
            if (
                (cur_state == state.IN_REP)
                and ((out_time - in_time) >= 4)
                and (global_counter >= 5)
            ):
                draw_str(
                    frame,
                    (20, 120),
                    "action %d: counting... %d" % (actions_counter, global_counter),
                    green_color,
                    2,
                )
