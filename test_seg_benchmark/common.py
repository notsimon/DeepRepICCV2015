    ...
    if not use_region_of_interest or framesstd == 0:
        ...
        for i in range(20):
            frameROIRes[i, :, :] = scipy.misc.imresize(
                frame_set[i, :, :], size=(50, 50), interp="bilinear"
            )
...
def load_next_test_data_wrapper(
    filename, stride, localization_method, segmentation_path=None
):
...
def load_next_test_data_simple_roi(fileName, stride, use_region_of_interest=True):
    ...
    while True:
        ...
        if len(framesList) > 20:
            ...
            # append sequence
            frames = np.reshape(
                frames, (1, frames.shape[0] * frames.shape[1] * frames.shape[2])
            )
...
def load_next_test_data_segmentation(fileName, segmentation_path, stride):
    ...
    masks_correct = np.loadtxt(
        "/home/tomrunia/data/VideoCountingDataset/QUVACount_Segments/localization/fast_video_segment_correct.txt"
    )
            ...
            if len(curr_frames) > 20:
                ...
                if debug:
                    ...
                    for show_idx in range(20):
                        ...
                        fast_seg.draw_box(
                            frame_draw,
                            block_boxes[show_idx],
                            color=(0, 255, 255),
                            thickness=1,
                        )
                        fast_seg.draw_box(
                            frame_draw, box, color=(0, 255, 255), thickness=2
                        )
                ...
                for i in range(20):
                    frames[i] = scipy.misc.imresize(
                        frames_crop[i, :, :], size=(50, 50), interp="bilinear"
                    )
