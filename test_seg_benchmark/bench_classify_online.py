...
def load_movie_data(fileName, localization_method, segmentation_path=None):
    ...
    (ns_test_set_x_st2, valid_st2) = load_next_test_data_wrapper(
        fileName, 2, localization_method, segmentation_path
    )
    (ns_test_set_x_st5, valid_st5) = load_next_test_data_wrapper(
        fileName, 5, localization_method, segmentation_path
    )
    (ns_test_set_x_st8, valid_st8) = load_next_test_data_wrapper(
        fileName, 8, localization_method, segmentation_path
    )

    return (
        (ns_test_set_x_st2, ns_test_set_x_st5, ns_test_set_x_st8),
        (valid_st2, valid_st5, valid_st8),
    )
...
def initial_count(classify, test_set_x, data, valid):
    ...
    (st2_count, st2_res, st2_entropy) = count_in_interval(
        classify, test_set_x, ns_test_set_x_st2, 0, 0, 81
    )  # 100 - 19 etc.

    # check if st5 is valid. if not return st2 count
    if valid_st5 == 1:
        (st5_count, st5_res, st5_entropy) = count_in_interval(
            classify, test_set_x, ns_test_set_x_st5, 0, 0, 21
        )
    else:
        st8_entropy = numpy.inf

    if valid_st8 == 1:
        (st8_count, st8_res, st8_entropy) = count_in_interval(
            classify, test_set_x, ns_test_set_x_st8, 0, 0, 6
        )
    else:
        st8_entropy = numpy.inf
...
def get_next_count(
    classify, test_set_x, data, valid, global_count, curr_residue, start_frame
):
    ...
    if winner == 0:
        # winner is stride 2
        return (
            global_count + st2_count,
            (st2_res * 2 / 2, st2_res * 2 / 5, st2_res * 2 / 8),
        )
    if winner == 1:
        # winner is stride 5
        return (
            global_count + st5_count,
            (st5_res * 5 / 2, st5_res * 5 / 5, st5_res * 5 / 8),
        )
    if winner == 2:
        # winner is stride 8
        return (
            global_count + st8_count,
            (st8_res * 8 / 2, st8_res * 8 / 5, st8_res * 8 / 8),
        )
...
def load_and_count_video(
    ...
    if data[0].shape[0] < 81:
        global_count = count_entire_movie(
            classify, test_set_x, data, valid, 0, (0, 0, 0), 0
        )
        return global_count
    ...
    for start_frame in range(200, 200 + (40 * numofiterations), 40):
        ...
        (global_count, curr_residue) = get_next_count(
            classify, test_set_x, data, valid, global_count, curr_residue, start_frame
        )

    # for frames that left get from each
    global_count = get_remain_count(
        classify,
        test_set_x,
        data,
        valid,
...
def test_benchmark_online(classify, test_set_x, batch_size):
    ...
    for nTestSet in range(tests_num):
        ...
        # Perform counting
        global_count = load_and_count_video(
            fileName, classify, test_set_x, batch_size, localization_method, seg_root
        )
